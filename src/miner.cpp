...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SecureCloudMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SecureCloudMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SecureCloudMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("securecloud-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SecureCloudMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
