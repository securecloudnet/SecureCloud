...
// Copyright (c) 2018-2019 The SecureCloud developers
...
/** Fees smaller than this (in uscn) are considered zero fee (for relaying and mining)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    CAmount nExpectedMint = GetBlockValue( (pindex->nHeight > 225000 ? pindex->nHeight : pindex->pprev->nHeight) );
    if (block.IsProofOfWork())
        nExpectedMint += nFees;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SCN: We use certain sporks during IBD, so check to see if they are
